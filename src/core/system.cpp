...
static u32 s_netplay_rollback_frames = 0;
...
bool System::DoState(StateWrapper& sw, GPUTexture** host_texture, bool update_display, bool is_memory_state)
{
  ...
  if (sw.IsReading() && !Netplay::Session::IsActive())
  {
    if (is_memory_state)
...
void System::CloseNetplaySession()
{
  if (!Netplay::Session::IsActive())
    return;

  s_netplay_states.clear();
  s_netplay_rollback_frames = 0;
  Netplay::Session::Close();
}

void System::SendNetplayMessage(std::string& message)
{
  if (!Netplay::Session::IsActive())
    return;

  Netplay::Session::SendMsg(message.c_str());
}

void System::NetplayLoop()
{
  // frame timing
...
bool _cdecl NpLoadFrameCb(void* ctx, uint8_t* buffer, int len, int rb_frames, int frame_to_load)
{
  ...
  SPU::SetAudioOutputMuted(true);
  u32 pred = Netplay::Session::GetMaxPrediction() + 2;
  bool result = System::LoadMemoryState(s_netplay_states[frame_to_load % pred]);
  s_netplay_rollback_frames = rb_frames;
  return result;
}
