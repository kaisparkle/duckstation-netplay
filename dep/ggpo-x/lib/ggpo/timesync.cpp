...
void TimeSync::advance_frame(GameInput& input, float advantage, float radvantage)
{
...
float TimeSync::recommend_frame_wait_duration(bool require_idle_input)
{
  ...
  // sleep for.
  /* char logMessage[256];
   snprintf <256>(logMessage, "Local Adv: %.2f, remoate adv %.2f", advantage, radvantage);
   OutputDebugString(logMessage);

   snprintf <256>(logMessage, ": Sleep for %.2f frames\n", sleep_frames);
   OutputDebugString(logMessage);
