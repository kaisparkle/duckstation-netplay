...
UdpProtocol::UdpProtocol()
  : _local_frame_advantage(0), _remote_frame_advantage(0), _queue(-1), _magic_number(0), _remote_magic_number(0),
    _packets_sent(0), _bytes_sent(0), _stats_start_time(0), _last_send_time(0), _shutdown_timeout(0),
    _disconnect_timeout(0), _disconnect_notify_start(0), _disconnect_notify_sent(false), _disconnect_event_sent(false),
    _connected(false), _next_send_seq(0), _next_recv_seq(0), _udp(NULL)
{
...
void UdpProtocol::Init(Udp* udp, Poll& poll, int queue, char* ip, u_short port, UdpMsg::connect_status* status)
{
  _udp = udp;
  _queue = queue;
...
void UdpProtocol::SendPendingOutput()
{
  ...
  if (_local_connect_status)
  {
    memcpy(msg->u.input.peer_connect_status, _local_connect_status,
           sizeof(UdpMsg::connect_status) * UDP_MSG_MAX_PLAYERS);
  }
  else
  {
...
#else
  strncpy(msg->u.chat.msg, message, MAX_CHAT_LENGTH);
#endif
  SendMsg(msg);
}
...
bool UdpProtocol::OnLoopPoll(void* cookie)
{
  ...
  switch (_current_state)
  {
    case Syncing:
      next_interval =
        (_state.sync.roundtrips_remaining == NUM_SYNC_PACKETS) ? SYNC_FIRST_RETRY_INTERVAL : SYNC_RETRY_INTERVAL;
    ...
    case Running:
      ...
      if (!_state.running.last_input_packet_recv_time ||
          _state.running.last_input_packet_recv_time + RUNNING_RETRY_INTERVAL < now)
      {
      ...
      if (!_state.running.last_quality_report_time ||
      ...
      if (!_state.running.last_network_stats_interval ||
      ...
      if (_disconnect_timeout && _disconnect_notify_start && !_disconnect_notify_sent &&
...
bool UdpProtocol::HandlesMsg(sockaddr_in& from, UdpMsg* msg)
{
  ...
  return _peer_addr.sin_addr.s_addr == from.sin_addr.s_addr && _peer_addr.sin_port == from.sin_port;
}
...
void UdpProtocol::OnMsg(UdpMsg* msg, int len)
{
  ...
  if (msg->hdr.type != UdpMsg::SyncRequest && msg->hdr.type != UdpMsg::SyncReply)
  {
...
void UdpProtocol::UpdateNetworkStats(void)
{
  ...
  Log("Network Stats -- Bandwidth: %.2f KBps   Packets Sent: %5d (%.2f pps)   "
      ...
      _kbps_sent, _packets_sent, (float)_packets_sent * 1000 / (now - _stats_start_time), total_bytes_sent / 1024.0,
      udp_overhead);
}
...
bool UdpProtocol::GetPeerConnectStatus(int id, int* frame)
{
...
void UdpProtocol::LogEvent(const char* prefix, const UdpProtocol::Event& evt)
{
  switch (evt.type)
  {
...
float UdpProtocol::RecommendFrameDelay()
{
...
void UdpProtocol::PumpSendQueue()
{
  while (!_send_queue.empty())
  {
    ...
    else
    {
      ...
      _udp->SendTo((char*)entry.msg, entry.msg->PacketSize(), 0, (struct sockaddr*)&entry.dest_addr,
                   sizeof entry.dest_addr);
  ...
  if (_oo_packet.msg && _oo_packet.send_time < Platform::GetCurrentTimeMS())
  {
    ...
    _udp->SendTo((char*)_oo_packet.msg, _oo_packet.msg->PacketSize(), 0, (struct sockaddr*)&_oo_packet.dest_addr,
                 sizeof _oo_packet.dest_addr);
