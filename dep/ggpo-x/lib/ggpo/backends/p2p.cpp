...
Peer2PeerBackend::Peer2PeerBackend(GGPOSessionCallbacks* cb, const char* gamename, uint16 localport, int num_players,
                                   int input_size, int nframes)
  : _num_players(num_players), _input_size(input_size), _sync(_local_connect_status, nframes),
    _disconnect_timeout(DEFAULT_DISCONNECT_TIMEOUT), _disconnect_notify_start(DEFAULT_DISCONNECT_NOTIFY_START),
    _num_spectators(0), _next_spectator_frame(0)
{
  _callbacks = *cb;
  _synchronizing = true;
...
void Peer2PeerBackend::CheckDesync()
{
    ...
    for (const auto& pair : ep._remoteCheckSums)
    {
      ...
      if (_confirmedCheckSums.count(checkSumFrame))
      {
        ...
        if (remoteChecksum != localChecksum)
        {
          ...
          char buf[256];
          snprintf(buf, 256, "DESYNC Checksum frame %d, local: %d, remote %d, size of checksum maps: %d,%d",
                   checkSumFrame, localChecksum, remoteChecksum, (int)_confirmedCheckSums.size(),
                   (int)ep._remoteCheckSums.size());
        ...
        {
          char buf[256];
          snprintf(buf, 256, "Checksum frame %d, local: %d, remote %d, size of checksum maps: %d,%d\n", checkSumFrame,
                   localChecksum, remoteChecksum, (int)_confirmedCheckSums.size(), (int)ep._remoteCheckSums.size());
          // OutputDebugStringA(buf);
        }
      }
    }
  ...
  {
    char buf[256];
    snprintf(buf, 256, "Erase checksums for frame %d\n", k);
    // OutputDebugStringA(buf);
...
GGPOErrorCode Peer2PeerBackend::DoPoll()
{
  // Pass on chat
    ...
    if (!_synchronizing)
    {
      ...
      if (current_frame > _next_recommended_sleep)
      {
        ...
        // if (interval > 0)
        {
          ...
          _callbacks.on_event(_callbacks.context, &info);
          _next_recommended_sleep =
            current_frame + RECOMMENDATION_INTERVAL; // RECOMMENDATION_INTERVAL;// RECOMMENDATION_INTERVAL;
        }
      }
    }
  }
...
GGPOErrorCode Peer2PeerBackend::AddLocalInput(GGPOPlayerHandle player, void* values, int size)
{
  int queue;
  GameInput input;
  ...
  { // xxx: <- comment why this is the case
    ...
    if (_confirmedCheckSumFrame >= 0)
    {
      ...
      snprintf(buf, 128, "Frame %d: Send checksum for frame %d, val %d\n", input.frame, _confirmedCheckSumFrame,
               input.checksum);
      // OutputDebugStringA(buf);
    }
...
GGPOErrorCode Peer2PeerBackend::SyncInput(void* values, int size, int* disconnect_flags)
{
  int flags;
...
GGPOErrorCode Peer2PeerBackend::IncrementFrame(uint16_t checksum1)
{
  ...
  if (_pendingCheckSums.count(_sync.GetFrameCount()))
  {
    ...
    snprintf(buf, 256, "Replace local checksum for frame %d: %d with %d, newest frame is %d, max diff %d\n",
             _sync.GetFrameCount(), oldChecksum, _pendingCheckSums[_sync.GetFrameCount()], max, maxDif);

    if (currentFrame <= _confirmedCheckSumFrame)
    {
      snprintf(buf, 256, "Changing frame %d in a rollback, but we've already sent frame %d\n", currentFrame,
               _confirmedCheckSumFrame);
      // OutputDebugStringA(buf);
      ASSERT(false);
    }
    if (diff >= (_sync.MaxPredictionFrames()))
    {
      ...
      snprintf(buf, 256, "diff is bigger than max prediction\n");
      // OutputDebugStringA(buf);
      ASSERT(false);
    }
  }
  ...
  {
    snprintf(buf, 256, "Added local checksum for frame %d: %d\n", _sync.GetFrameCount(), cSum);
    // OutputDebugStringA(buf);
  }
...
int Peer2PeerBackend::HowFarBackForChecksums() const
{
 ...
 uint16 Peer2PeerBackend::GetChecksumForConfirmedFrame(int frameNumber) const
 {
      ...
      {
          char s[128];
          snprintf <128>(s, "No local checksum found, remote frame is %d, adjusted is %d, most recent we have is %d\n",
 frameNumber, frameOfChecksumToSend, _checkSums.rbegin()->first); OutputDebugStringA(s); throw std::exception("s");
      }
      return _checkSums.at(frameOfChecksumToSend);
 }*/
void Peer2PeerBackend::OnUdpProtocolPeerEvent(UdpProtocol::Event& evt, int queue)
{
  ...
  switch (evt.type)
  {
    case UdpProtocol::Event::Input:
      if (!_local_connect_status[queue].disconnected)
      {
        ...
        {
          char buf[256];
          snprintf(buf, 256, "Received checksum for frame %d, remote cs is %d\n", checkSumFrame, remoteChecksum);
          // OutputDebugStringA(buf);
        }
      }
      break;
...
void Peer2PeerBackend::OnUdpProtocolSpectatorEvent(UdpProtocol::Event& evt, int queue)
{
 ...
 */
GGPOErrorCode Peer2PeerBackend::DisconnectPlayer(GGPOPlayerHandle player)
{
  int queue;
  GGPOErrorCode result;
...
void Peer2PeerBackend::DisconnectPlayerQueue(int queue, int syncto)
{
  GGPOEvent info;
...
GGPOErrorCode Peer2PeerBackend::GetNetworkStats(GGPONetworkStats* stats, GGPOPlayerHandle player)
{
  int queue;
  GGPOErrorCode result;
...
GGPOErrorCode Peer2PeerBackend::SetFrameDelay(GGPOPlayerHandle player, int delay)
{
  int queue;
  GGPOErrorCode result;
...
GGPOErrorCode Peer2PeerBackend::SetDisconnectTimeout(int timeout)
{
...
GGPOErrorCode Peer2PeerBackend::SetDisconnectNotifyStart(int timeout)
{
...
GGPOErrorCode Peer2PeerBackend::Chat(const char* text)
{
...
GGPOErrorCode Peer2PeerBackend::PlayerHandleToQueue(GGPOPlayerHandle player, int* queue)
{
