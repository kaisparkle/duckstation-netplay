...
GGPOErrorCode SyncTestBackend::DoPoll()
{
  if (!_running)
  {
    GGPOEvent info;
...
void SyncTestBackend::BeginLog(int saving)
{
  ...
  Platform::CreateDir("synclogs", NULL);
  snprintf(filename, ARRAY_SIZE(filename), "synclogs\\%s-%04d-%s.log", saving ? "state" : "log", _sync.GetFrameCount(),
...
void SyncTestBackend::LogSaveStates(SavedInfo& info)
{
  ...
  _callbacks.log_game_state(_callbacks.context, filename, _sync.GetLastSavedFrame().buf,
